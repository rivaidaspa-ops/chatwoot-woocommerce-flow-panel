...
function publicHealth(req, res) {
  res.json({ ok: true, service: 'chatwoot-woocommerce-flow-panel-v8.3.3-front-country-switch', port: PORT, redis: redisReady, postgres: dbReady, cache_items: memoryCache.size, sync: syncJob.running ? 'running' : 'idle' });
}
...
const server = app.listen(PORT, '0.0.0.0', () => console.log(`Panel v8.3.3 activo en puerto ${PORT}`));
